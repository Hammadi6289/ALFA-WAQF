...
const contentBlockSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      // GET ACTIVE BLOCKS
      .addCase(getActiveBlocks.pending, (state) => {
        state.loading = true;
      })
      .addCase(getActiveBlocks.fulfilled, (state, action) => {
        state.loading = false;
        state.blocks = action.payload.blocks || [];
      })
      .addCase(getActiveBlocks.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })

      // GET BLOCKS BY TYPE
      .addCase(getBlocksByType.pending, (state) => {
        state.loading = true;
      })
      .addCase(getBlocksByType.fulfilled, (state, action) => {
        state.loading = false;
        state.blocks = action.payload.blocks || [];
      })
      .addCase(getBlocksByType.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
